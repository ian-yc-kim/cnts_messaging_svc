...
async def websocket_endpoint(websocket: WebSocket, client_id: str):
    ...
    try:
        ...
        # Initialize activity timestamp explicitly
        try:
            manager.update_activity(client_id)
        except Exception:
            # Log and continue; connection is registered even if updating activity fails
            logger.exception(f"Failed to initialize activity for client {client_id}")
        ...
        while True:
            try:
                ...
                try:
                    ...
                    # Update activity on any successfully parsed client message
                    try:
                        manager.update_activity(client_id)
                    except Exception:
                        logger.exception(f"Failed to update activity for client {client_id} on message")
