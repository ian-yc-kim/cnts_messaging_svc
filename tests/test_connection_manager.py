...
class TestWebSocketConnectionManager:
    ...
    def test_connect_new_client(self, connection_manager, mock_websocket):
        ...
        # active_connections now stores (websocket, last_activity_at)
        ws, last_activity = connection_manager.active_connections[client_id]
    ...
    def test_connect_duplicate_client(self, connection_manager, mock_websocket, mock_websocket2):
        ...
        connection_manager.connect(mock_websocket, client_id)
        ws1, t1 = connection_manager.active_connections[client_id]
        assert ws1 is mock_websocket

        # Connect second websocket with same client_id
        connection_manager.connect(mock_websocket2, client_id)
        ws2, t2 = connection_manager.active_connections[client_id]
        assert ws2 is mock_websocket2
        assert connection_manager.get_connection_count() == 1
    ...
    def test_update_activity_advances_timestamp(self, connection_manager, mock_websocket):
        client_id = "client_activity"
        connection_manager.connect(mock_websocket, client_id)
        ws, ts1 = connection_manager.active_connections[client_id]
        # Artificially set an older timestamp
        old_time = datetime.utcnow() - timedelta(seconds=10)
        connection_manager.active_connections[client_id] = (ws, old_time)

        connection_manager.update_activity(client_id)
        ws2, ts2 = connection_manager.active_connections[client_id]
        assert ts2 > old_time
        assert ts2 <= datetime.utcnow()
